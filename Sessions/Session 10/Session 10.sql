...
/* ============================================================
   22. 🔁 استفاده از CTE بازگشتی برای نمایش مسیر دروس پیش‌نیاز
   ============================================================

   🎯 این کوئری مسیر کامل دروس و پیش‌نیازهای آن‌ها را به صورت سلسله‌مراتبی نمایش می‌دهد.
   🔁 از CTE بازگشتی استفاده شده تا ارتباط بین دروس (که یک ساختار درختی دارند) تحلیل شود.
   🧠 مسیر هر درس در ستون `Path` با استفاده از `CAST` و الحاق رشته‌ها ساخته می‌شود.
*/

;WITH cte AS (
	SELECT LessonID, LessonTitle, 1 AS Level, CAST(LessonTitle AS nvarchar(MAX)) AS Path
	FROM Lesson
	WHERE RequiredLessonID IS NULL

	UNION ALL

	SELECT L.LessonID, L.LessonTitle, Level + 1, Path + ' -> ' + L.LessonTitle
	FROM Lesson L
	INNER JOIN cte C ON C.LessonID = L.RequiredLessonID
)

SELECT *
FROM cte;

-- 🧩 این کوئری امکان مشاهده‌ی زنجیره‌ی پیش‌نیازها برای هر درس را فراهم می‌کند.
-- 📈 در آموزش‌هایی با دروس مرتبط به هم، برای طراحی درخت وابستگی بسیار کاربردی است.
-- 💡 توجه شود که پیوند درست بین `LessonID` و `RequiredLessonID` باید برقرار باشد تا حلقه ایجاد نشود.
